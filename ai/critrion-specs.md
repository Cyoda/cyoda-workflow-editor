...
- **Cross-source warnings.** If a workflow export contains an `operatorType` outside §3.1, surface it read-only and warn (don't silently drop or rewrite — preserve round-trip integrity for editing existing workflows that may pre-date this validation).
  - _Implemented (issue #22):_ the parse schema (`schema/criterion.ts`) accepts any non-empty operator string rather than the curated enum, so such a criterion no longer fails import. The semantic validator (`validate/semantic.ts`) emits a non-blocking `operator-not-recognized` warning for operators outside the known catalogue, and the existing `unsupported-operator` warning for known-but-engine-unimplemented operators (§3.2). The curated set is still the only one offered for authoring new criteria.
